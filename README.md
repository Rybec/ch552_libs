...
- [ ] main.c in ch55xduino/ch55x/cores/ch55xduino contains a bunch of ISR prototypes and definitions.  We don't want the user to have to include all of this code in main.c directly, so these ISR prototypes need to be moved to appropriate module headers, and the ISR definitions need to be moved to appropriate modules and then prototypes for them need to be added to the appropriate header files so they can cleanly be added to main.c without creating a ton of clutter.  (Done: USBDeviceInterrupt and timer0 interrupt.)


## Finished Work
- Renamed ch5xx.h to ch552.h.  Not sure that was needed, but if anything CH552 specific has or does end up in there, its name should not imply it will work for other CH5xx microcontrollers.
