...
// C preprocessor behavior is wierd, making this
// necessary when concatting arguments that themselves
// are #defined.
...
//### From Arduino.h ###
#define PROGMEM // Cancel PROGMEM, because it is only used in Atmel CPUs
...
SFR(CLOCK_CFG, 0xB9); // system clock config: lower 3 bits for system clock
                      // Fsys, Write@SafeMode
#define bOSC_EN_INT                                                            \
...
#define bROM_CLK_FAST                                                          \
  0x10 // flash-ROM clock frequency selection: 0=normal(for Fosc>=16MHz),
       // 1=fast(for Fosc<16MHz)
...
/*
   Fxt = 24MHz(8MHz~25MHz for non-USB application), from external oscillator
   @XI&XO Fosc = bOSC_EN_INT ? 24MHz : Fxt Fpll = Fosc * 4 => 96MHz
   (32MHz~100MHz for non-USB application) Fusb4x = Fpll / 2 => 48MHz (Fixed)
...
SBIT(MISO, 0x90,
     6); // master serial data input or slave serial data output for SPI0
SBIT(RXD1, 0x90, 6); // RXD input for UART1
SBIT(TIN4, 0x90, 6); // TIN4 for Touch-Key
SBIT(MOSI, 0x90,
     5); // master serial data output or slave serial data input for SPI0
...
SFR(P1_DIR_PU,
    0x93); // port 1 direction for push-pull or pullup enable for open-drain
...
SFR(P3_DIR_PU,
    0x97); // port 3 direction for push-pull or pullup enable for open-drain
...
#define bUSB_IO_EN                                                             \
  0x80 // USB UDP/UDM I/O pin enable: 0=P3.6/P3.7 as GPIO, 1=P3.6/P3.7 as USB
#define bIO_INT_ACT 0x40 // ReadOnly: GPIO interrupt request action status
#define bUART1_PIN_X                                                           \
  0x20 // UART1 alternate pin enable: 0=RXD1/TXD1 on P1.6/P1.7, 1=RXD1/TXD1 on
       // P3.4/P3.2
#define bUART0_PIN_X                                                           \
  0x10 // UART0 alternate pin enable: 0=RXD0/TXD0 on P3.0/P3.1, 1=RXD0/TXD0 on
       // P1.2/P1.3
#define bPWM2_PIN_X                                                            \
  0x08 // PWM2 alternate pin enable: 0=PWM2 on P3.4, 1=PWM2 on P3.1
#define bPWM1_PIN_X                                                            \
  0x04 // PWM1 alternate pin enable: 0=PWM1 on P1.5, 1=PWM1 on P3.0
#define bT2EX_PIN_X                                                            \
  0x02 // T2EX/CAP2 alternate pin enable: 0=T2EX/CAP2 on P1.1, 1=T2EX/CAP2 on
       // RST
#define bT2_PIN_X                                                              \
  0x01 // T2/CAP1 alternate pin enable: 0=T2/CAP1 on P1.1, 1=T2/CAP1 on P1.4
...
#define bPWM2_POLAR                                                            \
  0x40 // PWM2 output polarity: 0=default low and high action, 1=default high
       // and low action
#define bPWM1_POLAR                                                            \
  0x20 // PWM1 output polarity: 0=default low and high action, 1=default high
       // and low action
#define bPWM_IF_END                                                            \
...
SBIT(U1SM0, 0xC0,
     7); // UART1 mode, selection data bit: 0=8 bits data, 1=9 bits data
SBIT(U1SMOD, 0xC0,
     5); // UART1 2X baud rate selection: 0=slow(Fsys/32/(256-SBAUD1)),
         // 1=fast(Fsys/16/(256-SBAUD1))
SBIT(U1REN, 0xC0, 4); // enable UART1 receiving
SBIT(U1TB8, 0xC0, 3); // the 9th transmitted data bit in 9 bits data mode
SBIT(U1RB8, 0xC0, 2); // 9th data bit received in 9 bits data mode, or stop bit
                      // received for 8 bits data mode
SBIT(U1TI, 0xC0, 1);  // transmit interrupt flag, set by hardware after
                      // completion of a serial transmittal, need software clear
SBIT(U1RI, 0xC0, 0); // receive interrupt flag, set by hardware after completion
                     // of a serial receiving, need software clear
SFR(SBUF1,
    0xC1); // UART1 data buffer: reading for receiving, writing for transmittal
...
SBIT(CMP_IF, 0x80,
     6); // flag for comparator result changed, direct bit address clear
SBIT(ADC_IF, 0x80,
     5); // interrupt flag for ADC finished, direct bit address clear
SBIT(ADC_START, 0x80, 4); // set 1 to start ADC, auto cleared when ADC finished
SBIT(CMP_CHAN, 0x80,
     3); // comparator IN- input channel selection: 0=AIN1, 1=AIN3
...
#define bADC_EN                                                                \
  0x08 // control ADC power: 0=shut down ADC, 1=enable power for ADC
#define bCMP_EN                                                                \
  0x04 // control comparator power: 0=shut down comparator, 1=enable power for
       // comparator
#define bADC_CLK                                                               \
  0x01 // ADC clock frequency selection: 0=slow clock, 384 Fosc cycles for each
       // ADC, 1=fast clock, 96 Fosc cycles for each ADC
SFR(ADC_DATA, 0x9F); // ReadOnly: ADC data

/*  Touch-key timer Registers  */
SFR(TKEY_CTRL, 0xC3); // touch-key control
#define bTKC_IF                                                                \
  0x80 // ReadOnly: interrupt flag for touch-key timer, cleared by writing
       // touch-key control or auto cleared when start touch-key checking
...
#define bTKD_CHG                                                               \
  0x80 // ReadOnly: indicate control changed, current data maybe invalid

/*  USB/Host/Device Registers  */
SFR(USB_C_CTRL, 0x91); // USB type-C control
#define bVBUS2_PD_EN                                                           \
  0x80 // USB VBUS2 10K pulldown resistance: 0=disable, 1=enable pullup
#define bUCC2_PD_EN                                                            \
  0x40 // USB CC2 5.1K pulldown resistance: 0=disable, 1=enable pulldown
...
#define bVBUS1_PD_EN                                                           \
  0x08 // USB VBUS1 10K pulldown resistance: 0=disable, 1=enable pullup
#define bUCC1_PD_EN                                                            \
  0x04 // USB CC1 5.1K pulldown resistance: 0=disable, 1=enable pulldown
...
#define bUMS_DM_LEVEL                                                          \
  0x02 // ReadOnly: indicate UDM level saved at device attached to USB host
#define bUMS_DEV_ATTACH                                                        \
  0x01                 // ReadOnly: indicate device attached status on USB host
...
// sfr16 UH_RX_DMA     = 0xE4;         // host rx endpoint buffer start address,
// little-endian
#define UH_RX_DMA UEP2_DMA
// sfr UH_RX_DMA_L     = 0xE4;         // host rx endpoint buffer start address
// low byte
#define UH_RX_DMA_L UEP2_DMA_L
// sfr UH_RX_DMA_H     = 0xE5;         // host rx endpoint buffer start address
// high byte
#define UH_RX_DMA_H UEP2_DMA_H
// sfr16 UH_TX_DMA     = 0xE6;         // host tx endpoint buffer start address,
// little-endian
#define UH_TX_DMA UEP3_DMA
// sfr UH_TX_DMA_L     = 0xE6;         // host tx endpoint buffer start address
// low byte
#define UH_TX_DMA_L UEP3_DMA_L
// sfr UH_TX_DMA_H     = 0xE7;         // host tx endpoint buffer start address
// high byte
...
#define ROM_CHIP_ID_HX                                                         \
  0x3FFA // chip ID number highest byte (only low byte valid)
