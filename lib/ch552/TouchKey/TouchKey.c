...
#include "ch552/TouchKey/TouchKey.h"
...
uint8_t TouchKey_Process(void) { // call this function every 12ms or less.
  int8_t sampleToProcess = touchISRCounter - touchCounterProcessed;
  if (sampleToProcess > 0) {
    for (uint8_t i = 0; i < (uint8_t)sampleToProcess; i++) {
...
uint8_t TouchKey_Get(void) { return touchKeyPressed; }
